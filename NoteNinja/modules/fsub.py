...
from NoteNinja import BOT_ID
from NoteNinja import DRAGONS as DEVS
from NoteNinja import OWNER_ID
from NoteNinja import telethn as NoteNinja
from NoteNinja.events import NoteNinjainline
from NoteNinja.events import register as NoteNinjabot
from NoteNinja.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await NoteNinja(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await NoteNinja(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@NoteNinjabot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
...
@NoteNinja.on(events.NewMessage())
async def fsub_n(e):
...
@NoteNinjainline(pattern=r"fs(\_(.*))")
