...
from NoteNinja import DRAGONS
from NoteNinja.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def NoteNinja_is_admin(chat_id: int):
    status = False
    NoteNinja = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if NoteNinja.id == user.id:
            status = True
            break
    return status
